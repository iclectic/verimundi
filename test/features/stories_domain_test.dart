...
void main() {
  test(
    'underreported score rewards importance, local coverage, and international gap',
    () {
  ...
  test('story filtering searches and applies structured filters', () {
    ...
    final stories = [
      storyFixture(
        id: 'a',
        headline: 'Mobile clinics expand',
        tone: Tone.positive,
      ),
      storyFixture(
        id: 'b',
        headline: 'Water advisory',
        region: 'Europe',
  ...
  test('date formatter returns compact relative values', () {
    ...
    expect(
      DateFormatters.relative(now.subtract(const Duration(hours: 2)), now: now),
      '2h ago',
    );
  });
}
