      ...
      children: [
        SizedBox(
          ...
          child: SearchBar(
            ...
            onChanged: (value) =>
                ref.read(storyFilterProvider.notifier).state = filter.copyWith(query: value),
          ),
        ),
        DropdownMenu<String>(
          ...
          onSelected: (value) =>
              ref.read(storyFilterProvider.notifier).state = filter.copyWith(region: value),
          dropdownMenuEntries: const [
            ...
            DropdownMenuEntry(value: 'Latin America and the Caribbean', label: 'Latin America'),
        ...
        DropdownMenu<Severity>(
          ...
          onSelected: (value) =>
              ref.read(storyFilterProvider.notifier).state = filter.copyWith(severity: value),
        ...
        DropdownMenu<Tone>(
          ...
          onSelected: (value) =>
              ref.read(storyFilterProvider.notifier).state = filter.copyWith(tone: value),
          dropdownMenuEntries:
              Tone.values.map((item) => DropdownMenuEntry(value: item, label: item.name)).toList(),
        ),
        FilterChip(
          label: const Text('Local coverage'),
          selected: filter.hasLocalCoverage == true,
          onSelected: (selected) => ref.read(storyFilterProvider.notifier).state =
              filter.copyWith(hasLocalCoverage: selected ? true : null),
        ),
        TextButton.icon(
          onPressed: () => ref.read(storyFilterProvider.notifier).state = const StoryFilter(),
