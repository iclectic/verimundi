...
import 'package:verimundi/features/stories/domain/news_repository.dart';
...
void main() {
  test('mock AI summary handles insufficient evidence transparently', () async {
    ...
    final story = storyFixture(verificationStatus: VerificationStatus.insufficientEvidence);
    final result = await service.refreshSummary(story);

    expect(result.confidenceScore, 0);
    expect(result.shortSummary, contains('not currently have enough reliable source material'));
  });

  test('mock question service returns insufficient evidence for certainty prompts', () async {
    final service = MockStoryQuestionService();
    final answer = await service.askQuestion(storyFixture(), 'Can you prove this is certain?');
